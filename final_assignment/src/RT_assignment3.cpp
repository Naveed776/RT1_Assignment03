...
float x; // Goal coordinate x
float y; // Goal coordinate y
...
const char * menu = R"(
   ...
   -----------------------------------
   - Quit for the system press 0
   - Insert your new value of the target prees 1 
   - Cancel your current driving target press 2
   - for the Manual driving press 3
   -------------------------------------
)";
...
void currentSituation(const move_base_msgs::MoveBaseActionFeedback::ConstPtr& msg) {
	...
	distance_x = robotposition_x - x;
	distance_y = robotposition_y - y;
